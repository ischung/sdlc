...
description: >
  ...
  `append-issue` → `register-issues-to-github` (STEP 9 보드 반영 포함) 워크플로우로
  이슈를 만들도록 안내한다. 한 번 실행에 **이슈 1건만** 처리한다.
  🚫 **이슈 우선(Ticket-first) 가드** — 로컬 이슈 파일에만 있고 GitHub Issue 번호가
  부여되지 않은 이슈(`**GitHub Issue**: #N` 메타 부재) 는 구현 대상으로 받지 않는다.
  사용자가 파일의 로컬 순번(`#[N]`)이나 제목으로 픽업을 시도하면 `append-issue` →
  `register-issues-to-github` 실행을 먼저 안내하고 종료한다.
...
# 구현 담당 AI 개발자 스킬 — `implement-top-issue` v1.4
...
- **이슈 우선(Ticket-first) 가드**. 보드에 존재하고 GitHub Issue 번호가 부여된 이슈만 구현 대상이다. 로컬 파일의 `#[N]` 순번만으로는 구현을 시작하지 않는다. 해당 상황이 감지되면 `append-issue` → `register-issues-to-github` 실행을 안내하고 종료.
...
### STEP -1 — 이슈 우선(Ticket-first) 가드

호출 인자와 사용자 발화를 분석해 **"이 요청이 정식 이슈에 연결되어 있는가"** 를 먼저 검사한다.

```text
가드 판정 규칙 (하나라도 해당하면 즉시 차단):

  (G1) 사용자가 "이거 고쳐줘", "이 기능 만들어줘", "이 버그 잡아줘" 등
       구현 지시를 했는데 GitHub Issue 번호(#NNN)를 지목하지 않은 경우.
  (G2) --issue 인자가 로컬 이슈 파일의 `#[N]` 표기 형식으로 들어온 경우
       (예: --issue '#[7]'). 이는 GitHub 번호가 아님.
  (G3) 사용자가 파일명(final-issues.md 등)이나 제목 문자열만 제공한 경우.
  (G4) --issue N 이 제공됐으나 gh 저장소에서 존재하지 않거나
       해당 번호의 상태가 closed 인 경우.
  (G5) --issue N 이 제공됐으나 현재 보드의 Todo 컬럼에 없는 경우
       (STEP 1 강제 픽업 경로에서 이미 검증하지만, 여기서 선제 차단).
```

차단 시 다음 메시지를 출력하고 **구현 관련 어떤 명령도 실행하지 않고 종료**한다.

```text
🚫 이슈 우선(Ticket-first) 원칙 위반

이 프로젝트는 "이슈 없이 구현 금지" 를 강제합니다. 현재 요청은
GitHub Issue 번호와 보드 Todo 카드에 연결되어 있지 않습니다.

권장 복구 절차:

  1) 새 버그/기능이라면: /append-issue 로 이슈 파일에 추가
     → register-issues-to-github 가 자동으로 GitHub + 보드 반영
  2) 파일에는 있는데 GitHub 에 없는 이슈라면:
     /register-issues-to-github (STEP 9 포함) 로 일괄 반영
  3) 그 후 이 스킬을 다시 호출:
     /implement-top-issue                   (최우선 자동 픽업)
     /implement-top-issue --issue <GitHub번호>  (강제 지정)

⚠️ --issue 에는 반드시 **GitHub 의 실제 번호** (예: 67) 를 전달해야
   합니다. 로컬 파일의 `#[N]` 순번이 아닙니다.
```

인자 형식 정규화:

```bash
# --issue 값이 '#[N]' 같은 로컬 순번이면 즉시 차단
if [[ "${FORCE_ISSUE:-}" =~ ^#?\[[0-9]+\]$ ]]; then
  echo "🚫 --issue 값이 로컬 순번(#[N]) 형식입니다. GitHub 번호로 바꿔 주세요."
  exit 1
fi

# 숫자만 받도록 강제
if [ -n "${FORCE_ISSUE:-}" ] && ! [[ "$FORCE_ISSUE" =~ ^[0-9]+$ ]]; then
  echo "🚫 --issue 는 정수여야 합니다. 입력값: $FORCE_ISSUE"
  exit 1
fi
```

가드를 통과한 경우에만 STEP 0 으로 진입한다.
...
if [ -n "$FORCE_ISSUE" ]; then
  ...
  # (G4) 저장소에 존재하는 열린 이슈인지 먼저 확인
  EXIST_STATE=$(gh issue view "$ISSUE_NUMBER" --repo "$OWNER/$REPO" --json state -q .state 2>/dev/null || echo "MISSING")
  case "$EXIST_STATE" in
    MISSING)
      echo "🚫 #$ISSUE_NUMBER 이(가) 저장소에 존재하지 않습니다."
      echo "   → /append-issue 로 이슈 파일에 먼저 추가한 뒤 /register-issues-to-github 로 등록하세요."
      exit 1 ;;
    CLOSED)
      echo "🚫 #$ISSUE_NUMBER 은 이미 close 된 이슈입니다. 새 이슈로 등록하세요."
      exit 1 ;;
  esac

  # (G5) 해당 카드가 보드에 있는지 확인 (--argjson으로 안전 주입)
  ...
  if [ -z "$TOP_ITEM_JSON" ]; then
    echo "🚫 #$ISSUE_NUMBER 이(가) 보드(#$PROJECT_NUMBER) Todo 컬럼에 없습니다."
    echo "   복구: /kanban-add-issues 또는 /kanban-from-final-issues 로 보드에 먼저 올리세요."
    echo "   (이슈 우선 원칙: 보드에 없는 이슈는 구현 대상이 아닙니다.)"
    exit 1
  fi
else
...
---
🤖 Generated by \`implement-top-issue\` skill (v1.4)
EOF
)
...
18. **이슈 우선(Ticket-first) 가드**: STEP -1 에서 G1~G5 중 하나라도 해당하면 구현을 시작하지 않는다. `--issue` 에 로컬 순번(`#[N]`)이 들어오면 즉시 차단하고 GitHub 번호로 재시도하도록 안내. 이 가드가 "이슈 없이 구현 금지" 원칙의 **코드 레벨 enforcement** 이므로 어떤 `--force` 옵션으로도 우회하지 않는다.
...
| 상황 | 대응 |
| :---- | :---- |
| 'Todo'가 비어 있음 | 즉시 종료. `generate-issues-*` / `append-issue` → `register-issues-to-github` (STEP 9 포함) 안내. |
| 사용자가 GitHub 번호 없이 구현 요청 (G1) | STEP -1 차단. `/append-issue` 실행 권유 후 종료. 임의 추정 구현 금지. |
| `--issue` 에 `#[N]` 로컬 순번 전달 (G2) | STEP -1 차단. 해당 블록을 `/register-issues-to-github` 로 먼저 GitHub 에 올린 뒤 반환된 실제 번호로 재호출하도록 안내. |
| `--issue N` 이 저장소에 없음 (G4) | STEP 1 의 선제 검증에서 차단. `/append-issue` 로 이슈를 먼저 만들도록 안내. |
| `--issue N` 이 보드에 없음 (G5) | STEP 1 의 선제 검증에서 차단. `/kanban-add-issues` 또는 `/kanban-from-final-issues` 실행 안내. |
...
generate-issues-layered  ─┤                              │    (STEP -1 가드)      │    (일반)  ──→ 직접 구현          │
ci-cd-pipeline           ─┼─→ register-issues-to-github ─┤                        └─→ 검증·PR·Review 전이·런 모니터링 ─┘
append-issue (런타임)    ─┘   (STEP 9 = 칸반 반영 포함)
```

이슈 우선(Ticket-first) 원칙: 모든 작업은 왼쪽 열 → 가운데 → 오른쪽 순으로만 흐르며, 가운데 단계를 건너뛴 구현은 STEP -1 가드가 차단한다.
...
| 버전 | 변경 내용 |
| :---- | :---- |
| **v1.4** (현재) | **이슈 우선(Ticket-first) 가드 STEP -1 추가**. G1~G5 판정 규칙으로 "파일 순번만으로의 구현", "저장소에 없는 번호", "보드에 없는 번호" 를 선제 차단. `--issue` 인자 형식 검증(`#[N]` 거부, 정수만 허용). 행동 원칙 / 핵심 약속 / Safety Guards #18 / 실패 테이블에 원칙 명시. `append-issue` 신규 스킬을 이슈 만들기 단계에 편입하고 다이어그램 갱신. description 에 Ticket-first 가드 조항 추가. |
| **v1.3** | **CI/CD 하이브리드 모드 추가 (옵션 C)**. STEP 1.5에서 레이블/제목/본문 힌트로 CI/CD 성격 감지 → STEP 4에서 A/B/C 확인 후 `ci-cd-pipeline`에 구현 위임. STEP 5 검증 툴체인을 모드별로 스위치(일반: build/lint/test/Playwright, CI/CD: actionlint/yamllint/shellcheck/act/신규 워크플로우 ≥1). STEP 6.2 민감정보 체크를 파일 경로 기반으로 유지하고 `${{ secrets.X }}` 리터럴은 안전 문법으로 간주. STEP 7.5 추가: `gh run watch --exit-status`로 워크플로우 런 모니터링 + PR 코멘트. Safety Guards #16, #17 신설, 행동 원칙 #7에 CI/CD 모드 예외 명시. 인자 `--cicd-mode auto\|on\|off` 추가. |
